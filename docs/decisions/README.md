...
| `story-campaign-slots.md` | One campaign PER GOLFER (GS-story-campaign-slots): the `gs_story` roster, why a champion IS the completed slot rather than a snapshot, adopting a pre-roster save, the two store traps, and why `BACKUP_VERSION` went to 2 |
