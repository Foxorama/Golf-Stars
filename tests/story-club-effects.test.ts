...
describe('GS-story-club-effects — reward clubs carry a signature effect', () => {
  ...
  it("Voss's driver drives from any lie + raises the distance floor", () => {
    // GS-story-reward-variety: Dan's quest now gives an engine (a ship part), so the driver-from-any-lie
    // signature belongs to the Coil Apostate's own black driver.
    const s = equipStoryClub({ ...defaultStoryState(), ownedClubIds: ['quest:voss'] }, 'quest:voss');
    const out = applyStoryClubEffects(baseLoadout(), s);
  ...
  it('effects stack across multiple equipped reward clubs, and only when equipped (not merely owned)', () => {
    let s = { ...defaultStoryState(), ownedClubIds: ['quest:penelope', 'quest:sam'] };
    ...
    s = equipStoryClub(s, 'quest:sam');
    const out = applyStoryClubEffects(baseLoadout(), s);
    expect(out.greenRead).toBe(true); // Penelope's putter
    expect(out.puttBoost).toBeGreaterThan(0); // Penelope
    expect(out.dispersionMult).toBeLessThan(1); // Sam's Conviction — flies dead straight
  });
