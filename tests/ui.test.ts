...
describe('ui reducer', () => {
  ...
  it('a hole-in-one unlocks the secret Comet Rider ship, through the full play flow (GS-ace-ship)', () => {
    // Seed 25 (voyage, feather-fade) aces a hole on an early stop — drive the reducer to it and assert
    ...
    // GS-biome-difficulty, → 79 for GS-biome-variety, → 25 for GS-green-diversity: reflows of generated
    // layouts (here bigger greens + tucked pins) legitimately shift which seeded runs ace.)
    let s = started(25, 'voyage');
