...
describe('ui reducer', () => {
  ...
  it('a hole-in-one unlocks the secret Comet Rider ship, through the full play flow (GS-ace-ship)', () => {
    // Seed 107 (voyage, feather-fade) aces a hole on an early stop — drive the reducer to it and assert
    ...
    // → 699 for GS-fairway-width, → 107 for GS-compose: reflows of generated layouts legitimately
    // shift which seeded runs ace.)
    let s = started(107, 'voyage');
